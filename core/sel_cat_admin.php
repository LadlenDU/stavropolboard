...
if (ctype_digit(@$_REQUEST['id_root'])>0 && ctype_alpha($_REQUEST['id_place'])){
	...
	if (mysql_num_rows($query)){
		...
		$sub_cat="<select name=\"id_category\" id=\"id_category\" onchange=\"sel_cat(this.value,'".$place."');\"><option value=\"no\" selected=\"selected\">".$lang[537]."</option>";
...
while($sublist=mysql_fetch_assoc($query)){
			...
			if ($sublist['child_category']==0) $f="onclick=\"location.href='?action=ads&op=add_mess&cat='+this.value\""; else $f="";
			$sub_cat .= "<option value=\"".$sublist['id']."\" ".$f.">".$sublist[$name_cat]." ".@$arr."</option>";
