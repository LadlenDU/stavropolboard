<?php
	...
	$buttom_copyright = $settings['website_copyright'];

	//URL
	$script_url = ROOT_URL;

	//WEBSITE URL
	$website_url = str_replace("/sellbannermaster" , "" , ROOT_URL);
	...
	foreach($Pay->payments as $name => $pay) {
		if(file_exists(dirname(__FILE__)."/payments/".$pay->name."/lang/".$settings['website_language'].".php")) {
			require_once(dirname(__FILE__)."/payments/".$pay->name."/lang/".$settings['website_language'].".php");
		}
	}
