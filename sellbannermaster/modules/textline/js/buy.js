...
$(document).ready(function() {
	...
	if($('#title_length').is("span")) $('#title_length').text(title_length - $('input[name=textline_title]').val().length);
	if(typeof textline_id !== 'undefined') {
...
function show_timer() {
	$.ajax({
		type: "POST",
		url: script_url + "modules/textline/timer.php",
		data : "id=" + id + "&textline_id=" + textline_id,
...
function rebuild() {
	$.ajax({
		type: "POST",
		url: script_url + "modules/textline/rebuild.php",
		data : "textline_id=" + textline_id,
		success: function(status) {
...
function calculate_price() {
	price = 0;
	var textline_show_bought_count = $("input[name=textline_show_bought_count]").val() <= 0 ? 1 : $("input[name=textline_show_bought_count]").val();
	$("input[name=textline_show_bought_count]").val(textline_show_bought_count);
	if(price_1000 > 0) {
		price += price_1000 * textline_show_bought_count;
		$("#bought_count").text(textline_show_bought_count * 1000);
	}
	if(price_no_blank > 0 && $("input[name=textline_no_blank]:checked").length) {
	...
	$("#textline_price span").text(price);
}

function check_title_length(code) {
	var len = $("input[name=textline_title]").val().length;
...
function filled() {
	if($("input[name=textline_title]").val() == "") {
		alert(wrong_title);
		return false;
	} else if(!$("input[name=textline_terms]:checked").length) {
		alert(wrong_accept);
		return false;
	} else if(!isUrlValid($("input[name=textline_target_url]").val())) {
		alert(wrong_target);
		return false;
	} else if(!isValidEmailAddress($("input[name=textline_email]").val())) {
		alert(wrong_email);
		return false;
	} else if(price <= 0) {
		alert(wrong_data);
